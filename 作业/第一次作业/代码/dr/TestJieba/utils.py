...
def modify_file_column(dispose_list, function, dispose_column):
    ...
    modify_file = []
    for dispose_line in dispose_list: content = dispose_line.strip().split(",")
...
def build_match_label(list):
    ...
    if len(list[1]) != 0:
        for line in list[1]:
            var = line.strip().split('\t')
            keywords[var[1]] = str(keywords.get(var[1], 0)) + "|" + '.*' + var[0] + '.*'
    for key, value in keywords.items(): keywords[key] = value[2:]
    return keywords
...
def getFirstFile(type):
    ...
    if type == 'csv': fileName = GetFileWithType(sbasePath, type)[0]
    filePath = gbk_2_utf8(sbasePath) + '/' + fileName
    if type == 'txt': fileName = GetFileWithType(lbasePath, type)[0]
